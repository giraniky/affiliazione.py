...
"
        "- Oppure caricare un file Excel (.xlsx) con colonne 'ASIN', opzionali 'Marketplace' e 'Date'."
    )
...
async def process_asin_list(asin_lines, selected_marketplace, selected_date):
    ...
    if orders_data.get("success"):
        ...
        for asin, req in order_counts.items():
            ...
            if present < req:
                # recupera prezzo dal primo matching order
                price = 0.0
                for o in filtered:
                    if o.get("asin") == asin:
                        try:
